...
<?php
    ...
    if(!isset($_COOKIE['CR-userID']) || empty($_COOKIE['CR-userID'])){
        ...
        $query = "INSERT INTO request (`id_user`, `id_accessory`, `request_type`, `reservation_date`, `nb_rent_days`, `total_price`) VALUES ('$userid', '$accessory_id', 'accessory', '$reservation', '$rent_days', '$total')";
        ...
        // if($temp = @mysqli_query($conn, "CALL RequestInsertProcedure('$userid','$accessory_id','$reservation', '$rent_days', '$total')")){
        //     while($result = @mysqli_fetch_assoc($temp))
        //         if ($result != null) {
        //             print "<script>location.replace(\"reguest.php?id_accessory=".$result['LAST_INSERT_ID()']."\");</script>";
        //         }
        // }
...
<section class="bg-gray py-5">
    <div class="container">
        <form action="" enctype="multipart/form-data" method="POST">
            ...
            <!-- submit button -->
            <button type="submit" id="submit" name="submit" value="submit" class="btn btn-primary d-block mt-2">Rent accessory</button>
        </form>
    </div>
</section>
